...
export const paticipatedActivityByteacher = async (req, res) => {
  ...
  try {
    ...
    let targetDate = DateTime.now().setZone(zone);
    if (date) {
    ...
    if (activityPaticipate) {
      ...
      const rs = await db.activityParticipate.create({
        data: {
          activity: {
            connect: {
              actId: actId,
...
export const abstactActivityClassroom = async (req, res) => {
  ...
  if (!activityId && !classroomId)
    return res.status(401).json({ message: "Something error on Client side" });
  if (classroomId === "all");
...
export const abstactActivityFilterByRoom = async (req, res) => {
  ...
  try {
    ...
    const actDateStart = DateTime.fromISO(activitys.actDate.toISOString(), {
      zone: "UTC",
    }).setZone(zone);
    const actDateEnd = DateTime.fromISO(activitys.actDateEnd.toISOString(), {
      zone: "UTC",
    }).setZone(zone);
    ...
    const abstactFilterByClassroom = await classroomsHasMembers.reduce(
      async (prev, curr) => {
        const acc = await prev;
        const participateMember = Promise.all(
          curr.classroomMembers.map(async (member) => {
...
export const generateLinkActivityForQR = async (req, res) => {
  ...
  if (activityId) {
    try {
      ...
      const diff = activityDateEnd.diff(now, [
        "days",
        "hours",
        "minutes",
        "seconds",
...
export const saveActivityByStudentWithQR = async (req, res) => {
  ...
  if (token) {
    try {
      ...
      const activityDate = DateTime.fromISO(activity.actDate.toISOString(), {
        zone: "UTC",
      })
        .setZone(zone)
        .startOf("day");
        ...
        if (classroom) {
          ...
          if (!student) {
            return res.status(400).json({
              message: "you are not in classroom that can join this activity",
            });
          }
        }
      }
      ...
      if (activityParticipate) {
        ...
        await db.activityParticipate.create({
          data: {
            actId: activity.actId,
            stdId: req.user.id,
            joinTimestamp: DateTime.fromISO(activity.actDate.toISOString(), {
              zone: "UTC",
            }).setZone(zone),
            operateBy: "student",
          },
